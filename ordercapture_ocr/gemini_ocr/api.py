...
def _get_ocr_client() -> genai.Client:
	"""Lazy-init Gemini client — called per request to respect multitenancy."""
	api_key = frappe.db.get_single_value("Order Capture OCR Configuration", "model_secret")
	return genai.Client(api_key=api_key)


def _get_model_id() -> str:
	"""Lazy-init model ID — called per request to respect multitenancy."""
	# Options: "gemini-2.0-flash", "gemini-2.0-flash-lite-preview-02-05", "gemini-2.0-pro-exp-02-05"
	return frappe.db.get_single_value("Order Capture OCR Configuration", "gemini_model") or "gemini-2.0-flash"
...
class items(BaseModel):
	itemCode: str = Field(
		description="The item code, Extract the complete item code exactly as it appears, even if split across lines, also remove space if included. This is a critical identifier."
	)
	...
	totalAmount: float = Field(
		description="The total amount of the item, the amount could be in float or integer"
	)
...
class CustomerDetails(BaseModel):
	...
	customerAddress: Optional[str] = Field(
		description="The customer address in shipping, also check delivered to address in some cases, there might not be shipping address, so extract address from delivered to address if available"
	)
...
class Order(BaseModel):
	...
	orderDate: str = Field(
		description="The date of the order e.g. 2024-01-01, please change the date format to yyyy-mm-dd, also take note of formatting, not to mix up year and month, it could be dd-mm-yyyy or mm-dd-yyyy or yyyy-mm-dd, so make sure to extract the date in yyyy-mm-dd format"
	)
	orderExpiryDate: str = Field(
		description="The expiry date of the order e.g. 2024-01-01, please change the date format to yyyy-mm-dd, also take note of formatting, not to mix up year and month, it could be dd-mm-yyyy or mm-dd-yyyy or yyyy-mm-dd, so make sure to extract the date in yyyy-mm-dd format"
	)
...
def get_vendor_field_mappings(customer):
	...
	mapping_docs = frappe.get_all(
		"Vendor Field Mapping", filters={"customer": customer}, fields=["name"], limit=1
	)
	...
	field_mappings = frappe.get_all(
		...
		limit=500,  # nosemgrep: child table — bounded by parent doc
	)

	# Convert to dictionary for easy lookup, organized by model
	mapping_dict = {"items": {}, "Order": {}, "Totals": {}, "CustomerDetails": {}}

	for mapping in field_mappings:
		model_name = (
			mapping.model_name if mapping.model_name else "Order"
...
def update_model_descriptions(model_class, vendor_mappings, model_name):
	...
	updated_model = create_model(f"Custom{model_class.__name__}", **field_definitions, __base__=BaseModel)
...
@frappe.whitelist()
def extract_structured_data(file_path: str, customer: Optional[str] = None, model: BaseModel = Order):
	# Upload the file to the File API
	try:
		_client = _get_ocr_client()
		_model_id = _get_model_id()
		...
		if vendor_mappings:
			...
			custom_customer_details = update_model_descriptions(
				CustomerDetails, vendor_mappings, "CustomerDetails"
			)
			...
			for field_name, field_type in get_type_hints(Order).items():
				...
				order_field_definitions[field_name] = (
					field_type,
					Field(description=field_description),
				)

			# Create the custom Order model
			model = create_model("CustomOrder", **order_field_definitions, __base__=BaseModel)
			...
			for erpnext_field, vendor_field in vendor_mappings.get("items", {}).items():
				item_field_instructions.append(
					f"'{erpnext_field}' is referred to as '{vendor_field}' in their documents"
				)
		...
		frappe.throw(f"Error processing order: {e!s}")
