...
def update_model_descriptions(model_class, vendor_mappings, model_name):
    ...
    for field_name, field_type in model_annotations.items():
        ...
        if field_name_lower in model_mappings:
            ...
            field_description = (
                f"{field_description} (Vendor refers to this as '{vendor_field}')"
            )
    ...
    updated_model = create_model(
        f"Custom{model_class.__name__}", **field_definitions, __base__=BaseModel
    )
...
def extract_structured_data(file_path, customer=None, model: BaseModel = Order):
    ...
    try:
        ...
        if vendor_mappings:
            ...
            model = create_model(
                "CustomOrder", **order_field_definitions, __base__=BaseModel
            )
        ...
        if not os.path.exists(file_path):
            frappe.log_error(
                title="File Missing", message=f"File not found: {file_path}"
            )
        ...
        frappe.log_error(
            title="Error in extract_structured_data for gemini", message=str(e)
        )
