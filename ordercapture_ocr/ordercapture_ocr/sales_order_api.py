...
def get_customer_item_code(response):
	try:
		...
		# Batch-fetch all mappings before the loop (N+1 prevention)
		all_customer_codes = [item.get("itemCode") for item in response.get("orderDetails", [])]
		mapping_rows = frappe.get_all(
			"Customer Item Code Mapping",
			filters={"customer": customer_name, "customer_item_code": ["in", all_customer_codes]},
			fields=["customer_item_code", "item_code", "active"],
			limit=len(all_customer_codes) + 1,
		)
		mapping_lookup = {m.customer_item_code: m for m in mapping_rows}

		for item in response.get("orderDetails", []):
			customer_item_code = item.get("itemCode")
			mapping_data = mapping_lookup.get(customer_item_code)
			if mapping_data:
		...
		print(f"Error in get_customer_item_code: {e!s}")
...
def parse_iso_date(date_string):
	...
	try:
		...
		frappe.log_error(
			f"Failed to parse date: {date_string}, Error: {e!s}",
			"Date Parsing Error",
		)
		return None
...
def create_sales_order(
	response: str | dict, file_path: Optional[str] = None, ocr_doc_name: Optional[str] = None
):
	...
	try:
		...
		for item in response.get("orderDetails", []):
			...
			try:
				...
				if (
					document_plrate is not None
					and document_plrate > 0
					and abs(float(document_plrate) - float(system_plrate)) > 0.01
				):
				...
				frappe.log_error(
					f"Error getting item details for {mapped_item_code}: {e!s}",
					"Item Details Error",
				)
		...
		for so_item in sales_order.items:
			try:
				...
				system_plrate = system_item_details.get("price_list_rate", 0) or 0
				system_rate = (
					system_item_details.get("rate") or system_item_details.get("price_list_rate", 0) or 0
				)
		...
		if plrate_mismatches:
			try:
				...
				# Batch-fetch item names before loop (N+1 prevention)
				_item_codes = [m.get("item_code") for m in plrate_mismatches if m.get("item_code")]
				_item_name_map = {
					r.name: r.item_name
				...
				for mismatch in plrate_mismatches:
					...
					# Get item name from batch-fetched map
					item_code = mismatch.get("item_code")
					erpnext_item_name = _item_name_map.get(item_code) if item_code else None

					error_log = frappe.new_doc("OCR Error Log")
					error_log.update(
						{
