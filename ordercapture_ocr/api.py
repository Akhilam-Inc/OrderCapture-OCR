...
from ordercapture_ocr.ordercapture_ocr.ocr_processor.pdf_to_table import (
    pdf_tables_to_json,
)
from ordercapture_ocr.ordercapture_ocr.ocr_processor.structure_data_with_llm import (
    parse_purchase_order_with_llm,
)
...
def extract_purchase_order_data(file_path: str, vendor_type: str) -> dict:
    ...
    try:
        ...
        processors = {"BB": _process_bb_order, "FlipKart": _process_flipkart_order}
        ...
        return {
            ...
            "Customer": {
                "customerAddress": order_details.get("customer", {}).get(
                    "customer_address", ""
                ),
                "customerName": order_details.get("customer", {}).get(
                    "customer_name", ""
                ),
            },
            # 'orderDate': order_details.get('po_date', ''),
...
def _process_bb_order(df: pd.DataFrame) -> dict:
    ...
    po_number = (
        df.iloc[16, 0].split(":")[1].strip()
        if "PO Number" in str(df.iloc[16, 0])
        else ""
    )
    po_date = (
        df.iloc[16, 3].split(":")[1].strip() if "PO Date" in str(df.iloc[16, 3]) else ""
    )
    po_expiry_date = (
        df.iloc[16, 7].split(":")[1].strip()
        if "PO Expiry date" in str(df.iloc[16, 7])
        else ""
    )
...
def _extract_item_details(
    df: pd.DataFrame, start_row: int, end_row: int
) -> pd.DataFrame:
...
def process_file():
    ...
    try:
        ...
        frappe.throw(
            _("Error processing file. Please check file format and try again.")
        )
...
def process_spreadsheet(content, file_type):
    try:
        ...
        return {"type": "spreadsheet", "content": df.to_dict("records")}
...
def extract_excel_data(file_path):
    try:
        ...
        response = {"items": [], "addresses": {"delivery": {}, "company": {}}}

        # Extract item details
        items_df = df[
            ["item_code", "item_name", "qty", "rate"]
        ]  # Adjust column names as per excel structure
        ...
        delivery_df = df[
            ["delivery_name", "delivery_address", "delivery_city", "delivery_pincode"]
        ]
    ...
    if item_exists:
        ...
        try:
            ...
            frappe.log_error(
                ...
                "safe_get_item_details",
            )
            return {"price_list_rate": 0}
    else:
        # Item doesn't exist by code, try to find by name
        items = frappe.get_all(
            "Item", filters={"item_name": item_code}, fields=["name"]
        )
        ...
        if items and len(items) > 0:
            ...
            try:
                ...
                frappe.log_error(
                    f"Error getting item details by name for {item_code}: {str(e)}",
                    "safe_get_item_details",
                )
                return {"price_list_rate": 0}
        else:
            ...
            frappe.log_error(
                f"Item not found by code or name: {item_code}", "safe_get_item_details"
            )
            return {"price_list_rate": 0}
...
def set_value(doctype, name, fieldname):
    ...
    if not frappe.has_permission(
        doctype=doctype, ptype="write", doc=frappe.get_doc(doctype, name)
    ):
        frappe.throw(_("Not permitted"), frappe.PermissionError)
...
def prepare_dynamic_prompt():
    ...
    try:
        ...
        # Build the prompt
        prompt = (
            "Extract the following information from the purchase order document:\n\n"
        )
        ...
        for vendor, mappings in vendors.items():
            ...
            for mapping in mappings:
                prompt += (
                    f"- '{mapping['field_name']}' maps to '{mapping['target_field']}'"
                )
