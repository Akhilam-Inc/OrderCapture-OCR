...
def extract_purchase_order_data(file_path: str, vendor_type: str) -> dict:
	...
	try:
		...
		frappe.log_error(title="Purchase Order Processing Error", message=frappe.get_traceback())
		frappe.throw(f"Error processing purchase order: {e!s}")
...
def _process_bb_items(item_details: pd.DataFrame) -> list:
	...
	items = []
	for _index, row in item_details.iterrows():
		item = {
...
def process_file():
	...
	try:
		...
		frappe.log_error(title="File Processing Error", message=frappe.get_traceback())
...
def process_pdf(content):
	...
	try:
		...
		frappe.log_error(title="PDF Processing Error", message=frappe.get_traceback())
		raise
...
def process_spreadsheet(content, file_type):
	try:
		...
		frappe.log_error(title="Spreadsheet Processing Error", message=frappe.get_traceback())
		raise
...
def extract_excel_data(file_path):
	try:
		...
		frappe.log_error(title="Excel Extraction Error", message=frappe.get_traceback())
		return None
...
def get_item_details_with_fallback(args):
	...
	if item_exists:
		...
		try:
			...
			frappe.log_error(
				f"Error getting item details for {item_code}: {e!s}",
				"safe_get_item_details",
			)
			return {"price_list_rate": 0}
	else:
		# Item doesn't exist by code, try to find by name
		items = frappe.get_all("Item", filters={"item_name": item_code}, fields=["name"], limit=1)
		...
		if items and len(items) > 0:
			...
			try:
				...
				frappe.log_error(
					f"Error getting item details by name for {item_code}: {e!s}",
					"safe_get_item_details",
				)
				return {"price_list_rate": 0}
		else:
...
def prepare_dynamic_prompt():
	...
	try:
		...
		vendor_mappings = frappe.get_all(
			...
			filters={"enabled": 1},
			limit=500,  # nosemgrep: config table — bounded by design
		)
		...
		frappe.log_error(title="Error Preparing Dynamic Prompt", message=frappe.get_traceback())
		return get_default_prompt()
