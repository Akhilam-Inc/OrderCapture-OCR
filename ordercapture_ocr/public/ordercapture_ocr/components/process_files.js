...
ordercapture_ocr.process_dialog = {
  show(docId) {
    ...
    d.events.post_sales_order = function() {
      ...
      const item_customer = d.get_value('customer');
      ...
      // After creating items, proceed with SO creation
      Promise.all(items_data.map(item => 
        frappe.db.get_value('Item', {'item_code': item.itemCode}, ['name'])
          .then(result => {
                ...
                return frappe.call({
                    ...
                    args: {
                        doc: {
                            ...
                            item_group: 'Products', // Set your default item group
            ...
            .then(() => {
              // Create customer item code mapping
              return frappe.call({
                  method: 'frappe.client.insert',
                  args: {
                      doc: {
